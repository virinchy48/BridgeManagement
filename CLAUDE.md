...
[2026-05-22] [BTP / HTML5Runtime_enabled / Trial Broker] Learning: The `HTML5Runtime_enabled: true` parameter in `mta.yaml` under the `BridgeManagement-app-deployer` → `BridgeManagement-html5-repo-host` requires block causes ALL HTML5 apps to return "Application not found { CODE: '5057' }" from the RT service in SAP BTP trial environments. Root cause: the MTA deployer tries to create a service key with `HTML5Runtime_enabled: true`; the trial broker rejects it with "html5_app_configuration/HTML5Runtime_enabled: Additional properties not allowed"; the deployer creates the key without the flag; HTML5 content is uploaded to storage but NOT registered as runtime-accessible in the RT service's discovery index. Fix: remove the `config: HTML5Runtime_enabled: true` block entirely from mta.yaml. In BTP trial (and likely all single-tenant deployments), the RT service automatically discovers ALL content uploaded to any app-host service instance in the same subaccount — no explicit flag needed. `HTML5Runtime_enabled` is only relevant for multi-tenant SaaS subscriber scenarios. After removing the block and redeploying, all 10 HTML5 app manifests returned HTTP 200.
Source: v2.0.5 → v2.0.6 investigation — all tiles "App could not be opened" after every deploy
Applied: mta.yaml — removed config.HTML5Runtime_enabled block from BridgeManagement-app-deployer requires; bumped to v2.0.6
