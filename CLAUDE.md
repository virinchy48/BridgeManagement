...
[2026-05-23] [BTP / html5-apps-repo-rt silent failure / localDir bypass] Learning: On BTP trial, the html5-apps-repo-rt service can silently fail to register app content from the HOST service even when: (1) the HOST service is in "create succeeded" state; (2) the MTA deploy reports CODE 1002 "Application content was successfully uploaded from subaccount"; (3) the MTAR data.zip contains correctly built apps with valid sap.app.id values. Diagnostic: all app URLs return HTTP 503 (596 bytes OpenResty HTML). Fix: bypass html5-apps-repo-rt entirely by extracting all UI5 app bundles (from the MTAR data.zip) into `app/router/{AppNamespace}/` and adding a single xs-app.json localDir route: `{"source": "^/{AppNamespace}/(.*)$", "localDir": ".", "authenticationType": "xsuaa"}` — this route must appear BEFORE the html5-apps-repo-rt catch-all route. The approuter maps `/AppNamespace/appname/file.js` to `./AppNamespace/appname/file.js` within its root directory. The `{AppNamespace}` prefix must match the first path segment of `sap.app.id` (e.g. `sap.app.id = "BridgeManagementV2.adminbridges"` → extract to `app/router/BridgeManagementV2/adminbridges/`). Total uncompressed size of all 10 BMS apps is ~4.9MB, well within CF app limits. After fix: all app URLs return HTTP 200 (XSUAA redirect, ~741 bytes) instead of 503. The html5-apps-repo-rt catch-all route at the end of xs-app.json is harmless to leave in place for future use.
Source: BridgeManagementV2 BTP deployment — all tiles "App could not be opened" after CODE 1002 success — 2026-05-23
Applied: app/router/xs-app.json + app/router/BridgeManagementV2/ (302 files) — BridgeManagementV2 v2.0.9

[2026-05-23] [BTP / UI5 Component-preload.js priority] Learning: UI5 loads `Component-preload.js` BEFORE `Component.js` for performance. If `Component.js` is patched with the correct path but `Component-preload.js` still has old hardcoded paths, the app loads the OLD preload and the fix has no effect. The preload contains an inline copy of Component logic (`sap.ui.predefine("Namespace/Component", ...)`). Always patch BOTH `Component.js` AND `Component-preload.js` AND `Component-dbg.js` when fixing hardcoded paths. Pattern: check for old strings in all three files: `grep "old-path" Component.js Component-preload.js Component-dbg.js`.
Source: v2.0.11 → v2.0.12 fix — "sap logo still having issue" after Component.js was patched
Applied: app/router/BridgeManagementV2/adminbridges/Component-preload.js + Component-dbg.js

[2026-05-23] [BTP / sap.ui.require.toUrl for dynamic script loading] Learning: Any `loadScript()` pattern that sets `script.src` to a hardcoded absolute path (e.g. `"/admin-bridges/webapp/ext/controller/gisMapInit.js"`) breaks on BTP because the BTP localDir bypass serves apps under `/BridgeManagementV2/adminbridges/`, not `/admin-bridges/webapp/`. Fix: use `sap.ui.require.toUrl("BridgeManagementV2/adminbridges/ext/controller/")` as the base URL — this dynamically resolves to the correct path in both local dev (`/admin-bridges/webapp/ext/controller/`) and BTP (`/BridgeManagementV2/adminbridges/ext/controller/`). This fix must be applied to EVERY app that has a Component.js with dynamic script loading, not just adminbridges. Check ALL Component.js files: `grep -r "loadScript\|script.src = '" app/*/webapp/Component.js`.
Source: v2.0.12 — restrictions Component.js also had hardcoded `/restrictions/webapp/ext/controller/NumericInputGuard.js`
Applied: app/restrictions/webapp/Component.js, app/admin-bridges/webapp/Component.js

[2026-05-23] [BTP / inline script src in fragment XML] Learning: `<core:HTML>` fragments that set `script.src` via inline JavaScript use relative URLs resolved against the PAGE URL, not the fragment file URL. `src = '../webapp/ext/controller/MyFile.js'` on a page at `https://host/fiori-apps.html` resolves to `https://host/webapp/ext/controller/MyFile.js` — wrong in both local dev and BTP. Fix: use `sap.ui.require.toUrl('Namespace/appname/ext/controller/MyFile') + '.js?' + Date.now()` inside the inline script. Since fragments render after UI5 is loaded, `sap` is always available in the inline script context.
Source: v2.0.12 — restrictions CustomAttributes.fragment.xml had `'../webapp/ext/controller/CustomAttributesRestrInit.js'`
Applied: app/restrictions/webapp/ext/fragment/CustomAttributes.fragment.xml + deployed equivalent

[2026-05-23] [BTP / HANA auto-stop recovery] Learning: BTP trial HANA free-tier auto-stops after ~30 minutes of inactivity. Symptom in CF logs: `TimeoutError: ResourceRequest timed out at ResourceRequest._fireTimeout` in BridgeManagementV2-srv. Recovery: `cf update-service Hanaclouddb -c '{"data":{"serviceStopped":false}}'` then poll `cf service Hanaclouddb` until `last operation: update succeeded` (~3 minutes). Then `cf restart BridgeManagementV2-srv` to re-establish the connection pool. Do not restart the approuter — it is not affected by HANA stops.
Source: UAT session 2026-05-23 — "ResourceRequest timed out ON SELECTION OF BRIDGE REGISTER"
Applied: Recovery procedure documented
...
[2026-05-23] [Map View / Demo Mode] Learning: `demoModeActive` was removed from the backend but `app/map-view/webapp/controller/Main.controller.js` `_loadGisConfig` still fetched `SystemConfig('demoModeActive')` via `demoP` inside `Promise.all([configP, demoP])`. The fix was to remove `demoUrl`, `demoP`, and `demoModeConfig` references entirely and convert `Promise.all` back to a plain `configP.then(...)` chain. This pattern repeats: whenever a feature is removed from the backend, grep ALL app controller files for the feature's key/endpoint — not just the most likely location. The original CLAUDE.md entry for P3-001 incorrectly identified `Shell.controller.js` as the source; the actual location was `Main.controller.js` in map-view.
Source: UAT fix sprint 2026-05-23 — P3-001
Applied: app/map-view/webapp/controller/Main.controller.js — demoP fetch + demoModeConfig removed

[2026-05-23] [AdminService / BridgeMaintenanceActions / Reverse FK resolution] Learning: The `before(['CREATE','UPDATE'], BridgeMaintenanceActions)` handler in `srv/admin/maintenance.js` only resolved `bridge_ID` FROM `bridgeRef` (one direction). When records are created via FE4 ObjectPage value-help (which sets `bridge_ID` via the association picker but leaves `bridgeRef` null), the Work Orders list report shows a blank Bridge ID column because the annotation uses `bridgeRef` (the human-readable string). Fix: add an `else if (d.bridge_ID)` branch that resolves `bridgeRef` from `bridge_ID` — `SELECT.one.from(Bridges).columns('bridgeId').where({ID: d.bridge_ID})` → `d.bridgeRef = bridge.bridgeId`. This bi-directional resolution pattern must be applied to ANY sub-domain entity that uses a `bridgeRef` column in its `@UI.LineItem` annotation but receives `bridge_ID` from the FE4 value-help picker.
Source: UAT fix sprint 2026-05-23 — P2-001
Applied: srv/admin/maintenance.js

[2026-05-23] [FE4 / i18n supportedLocales] Learning: UI5 automatically requests locale-variant property files (`i18n_en_GB.properties`, `i18n_en.properties`) for every app on load. Since only `i18n.properties` (default) exists, these always 404. The 404s are harmless but pollute the network log. Fix: add `"settings": { "supportedLocales": [""] }` to every app's i18n model entry in `manifest.json` (`sap.ui5.models` section, the model with `uri` pointing to `i18n/i18n.properties`). The empty-string locale `""` means "only load the default file". Must be updated in ALL app manifests simultaneously — missing any one leaves 404s for that app.
Source: UAT fix sprint 2026-05-23 — P3-002
Applied: All 11 webapp manifest.json files

[2026-05-23] [FE4 / CriticalityCalculation vs Criticality on ObjectPage header chips] Learning: `CriticalityCalculation` (with `ImprovementDirection`, `ToleranceRangeLowValue`, `DeviationRangeLowValue`) does NOT apply colour-coding to DataPoint chips rendered in `HeaderFacets` of an FE4 ObjectPage — the chip renders in plain black regardless. Only a direct integer `Criticality: <virtualIntegerField>` binding applies the red/amber/green colour. Pattern: add a `virtual conditionRatingCriticality : Integer` field to the entity CDS (same as `postingStatusCriticality`); compute it in `after('READ', Bridges)` handler (1=red <5, 2=amber 5-7, 3=green ≥8); annotate with `@UI.Hidden`; reference as `Criticality: conditionRatingCriticality` in the DataPoint annotation. This applies to ALL numeric DataPoint chips that need threshold-based colouring in ObjectPage headers.
Source: UAT fix sprint 2026-05-23 — condition rating chip showing black
Applied: db/schema/bridge-entity.cds, srv/admin-service.js, app/admin-bridges/annotations/bridges.cds

[2026-05-23] [Seed Data / BridgeScourAssessments active field] Learning: `BridgeScourAssessments` entity does NOT have an `active: Boolean` field — it has no soft-delete lifecycle. Do not include an `active` column in `bridge.management-BridgeScourAssessments.csv`. Including it causes `cds deploy` to fail with SQLITE_ERROR because the INSERT column count (including `active`) exceeds the schema column count. The entities that DO have `active` are: BridgeInspections, BridgeDefects, Restrictions, BridgeCapacities, BridgeConditionSurveys, BridgeLoadRatings, BridgePermits, BridgeMaintenanceActions. Always verify entity schema before writing seed CSV columns.
Source: Seed data sprint 2026-05-23 — cds deploy failure on BridgeScourAssessments
Applied: db/data/bridge.management-BridgeScourAssessments.csv — removed `active` column

[2026-05-23] [Seed Data / Sub-domain entities] Learning: All 8 sub-domain entities that were previously empty now have seed data (39 rows total). Reference bridge_IDs used: 1 (Lennox Bridge), 2 (Hampden Bridge), 3 (Grafton Bridge), 4 (Burdekin Bridge), 5 (Sydney Harbour Bridge), 7 (Narrows Bridge), 8 (Mooney Mooney Creek), 9 (Story Bridge). Auto-ref sequences start at 0001 (CS-0001, LR-0001, PM-0001, SAR-0001, NRA-0001, RSK-0001, MA-0001) — the `before('NEW', Entity.drafts)` handler continues from the highest existing ref. Never include managed mixin fields (createdAt, createdBy, modifiedAt, modifiedBy) or virtual fields (bsiScore, conditionRatingCriticality, etc.) in seed CSVs — CDS rejects unknown columns during deploy.
Source: Seed data sprint 2026-05-23
Applied: 8 new db/data/bridge.management-*.csv files committed in c5b6d51
