sap.ui.define([
  ...
  return Controller.extend("BridgeManagement.bnacconfig.controller.Main", {
    ...
    onInit: function () {
      ...
      // Create native file input dynamically (cannot put <input> in XML view)
    ...
    onEditEnv: function (oEvent) {
      const ctx = oEvent.getSource().getBindingContext("envModel");
      this._openEnvDialog(ctx.getObject());
    },

    onDeleteEnv: function (oEvent) {
      const ctx   = oEvent.getSource().getBindingContext("envModel");
      const env   = ctx.getObject().environment;
    ...
    _openEnvDialog: function (existing) {
      ...
      const form = new SimpleForm({ layout: "ColumnLayout", columnsM: 1, editable: true, content: [
