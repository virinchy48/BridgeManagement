(function (factory) {
  ...
  simpleheat.prototype = {
    defaultRadius: 25,
    defaultGradient: { 0.4: '#3b82f6', 0.6: '#22c55e', 0.7: '#eab308', 0.8: '#f97316', 1.0: '#ef4444' },
    ...
    _createCanvas: function () {
      return typeof document !== 'undefined' ? document.createElement('canvas') : new this._canvas.constructor();
    }
  };
  ...
  var HeatLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    _redraw: function () {
      ...
      for (var yi = 0, ylen = grid.length; yi < ylen; yi++) {
        if (grid[yi]) {
          for (var xi = 0, xlen = grid[yi].length; xi < xlen; xi++) {
            var cell2 = grid[yi][xi];
            if (cell2) {
              data.push([Math.round(cell2[0]), Math.round(cell2[1]), Math.min(cell2[2], max)]);
            }
          }
        }
      }
