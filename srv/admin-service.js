...
module.exports = class AdminService extends cds.ApplicationService { init() {
  ...
  // ── Domain modules — one file per sub-domain tile ───────────────────────
  // To find handlers for a specific tile, read the matching srv/admin/*.js file.
  const helpers = { diffRecords, writeChangeLogs, fetchCurrentRecord }

  require('./admin/attributes')(this)
  require('./admin/inspections')(this, helpers)
  require('./admin/defects')(this, helpers)
  require('./admin/risk-assessments')(this, helpers)
  require('./admin/load-rating-certs')(this, helpers)
  require('./admin/nhvr')(this, helpers)
  require('./admin/conditions')(this, helpers)
  require('./admin/load-ratings')(this, helpers)
  require('./admin/permits')(this, helpers)
  require('./admin/asset-iq')(this)
  require('./admin/maintenance')(this)
  ...
  this.after('UPDATE', GISConfig, async (_result, req) => {
    ...
    if (!changes.length) return
    await writeChangeLogs(db, {
      objectType: 'GISConfig', objectId: 'default', objectName: 'GIS Configuration',
      source: 'OData', batchId: cds.utils.uuid(), changedBy: req.user?.id || 'system', changes
    })
  })

  // Sync scourRisk back to parent Bridge on scour assessment create/update
  this.after(['CREATE', 'UPDATE'], BridgeScourAssessments, async (data) => {
    if (!data?.bridge_ID || !data?.scourRisk) return
    const db = await cds.connect.to('db')
    await db.run(UPDATE('bridge.management.Bridges').set({ scourRisk: data.scourRisk }).where({ ID: data.bridge_ID }))
  })
