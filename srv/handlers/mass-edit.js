...
module.exports = function registerMassEditHandlers (srv, { logAudit, validateEnum }) {
    ...
    srv.on('massEditBridges', async req => {
        ...
        for (let [i, row] of rows.entries()) {
            try {
                if (!row.ID) { failed++; errors.push(`Row ${i+1}: ID required`); continue }
                let current = await db.run(SELECT.one.from('nhvr.Bridge').where({ ID: row.ID }))
                if (!current) { failed++; errors.push(`Row ${i+1}: Bridge ${row.ID} not found`); continue }
                if (row.version !== undefined && current.version !== row.version) {
                    failed++; errors.push(`Row ${i+1}: Version conflict for ${current.bridgeId}`); continue
                }
                let patch = {}
                GRID_FIELDS.forEach(f => { if (row[f] !== undefined) patch[f] = row[f] })
                if (patch.conditionRating) {
                    let r = parseInt(patch.conditionRating)
                    if (r < 1 || r > 10) { failed++; errors.push(`Row ${i+1}: conditionRating must be 1-10`); continue }
                    patch.condition = CONDITION_LABELS[r]
                    patch.conditionScore = Math.round((r / 10) * 100)
                    patch.highPriorityAsset = r <= 4
                }
